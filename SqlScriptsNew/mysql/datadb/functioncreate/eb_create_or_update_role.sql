...
CALL eb_str_to_tbl_util(permissions_str,',');  
...
INSERT INTO 
	...
	SELECT 
         `value`, rid, createdby, NOW(), 
			CAST( eb_split_str_util(`value`,'-',4) AS UNSIGNED INT),
			CAST( eb_split_str_util(`value`,'-',3) AS UNSIGNED INT)
    FROM 
		( (SELECT `value` FROM permissions_tmp WHERE `value` NOT IN 
