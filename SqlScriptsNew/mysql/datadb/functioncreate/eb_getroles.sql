...
SELECT 	
	...
	FROM 
		(
			SELECT 
					role_id, r.role_name, r.applicationid 
				FROM
				(
					SELECT 
							DISTINCT ROLES.role_id 
						FROM
						(
							SELECT 
									er2u.role_id 
								FROM 
									eb_role2user er2u
								WHERE 
									er2u.user_id = userid AND er2u.eb_del = 'F'
							UNION ALL
							(
											...
											role1_id = ANY(
												SELECT 
													...
													FROM 
														eb_role2user 
													WHERE 
														user_id = userid AND eb_del = 'F') 
											AND eb_del = 'F'
										UNION ALL
											SELECT 
													e.role2_id 
												FROM 
													eb_role2role e, role2role r 
												WHERE 
													e.role1_id = r.role_id AND eb_del='F'
		...
		LEFT JOIN
			(
				SELECT * 
					FROM 
						eb_roles er 
					WHERE er.applicationid = ANY(
						SELECT 
								ea.id 
							FROM 
								eb_applications ea 
							WHERE 
								FIND_IN_SET(ea.application_type,app_type)) 
					AND er.eb_del = 'F' ) r
			ON
				qury.role_id = r.id
		) UROLES
