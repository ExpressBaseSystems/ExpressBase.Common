...
BEGIN
    ...
    IF in_id > 1 THEN
        ...
        UPDATE eb_role2user SET eb_del = 'T',revokedby = in_userid,revokedat = SYSTIMESTAMP 
            WHERE role_id IN(SELECT TO_CHAR(role_id) from eb_role2user WHERE user_id = in_id AND eb_del = 'F' MINUS SELECT regexp_substr(in_roles_temp,'[^,]+', 1, level) from dual CONNECT BY regexp_substr(in_roles_temp, '[^,]+', 1, level) is not null);
        UPDATE eb_user2usergroup SET eb_del = 'T',revokedby = in_userid,revokedat = SYSTIMESTAMP WHERE groupid IN(SELECT TO_CHAR(groupid) from eb_user2usergroup WHERE userid = in_id AND eb_del = 'F' MINUS SELECT regexp_substr(in_group_temp,'[^,]+', 1, level) from dual CONNECT BY regexp_substr(in_group_temp, '[^,]+', 1, level) is not null);
        IF in_roles_temp IS NOT NULL THEN
            INSERT INTO eb_role2user(role_id,user_id,createdby,createdat) SELECT roleid, in_id,in_userid,SYSTIMESTAMP FROM 
                (SELECT regexp_substr(in_roles_temp,'[^,]+', 1, level) AS roleid from dual CONNECT BY regexp_substr(in_roles_temp, '[^,]+', 1, level) is not null MINUS SELECT TO_CHAR(role_id) from eb_role2user WHERE user_id = in_id AND eb_del = 'F');
        END IF;        
        IF in_group_temp IS NOT NULL THEN
            INSERT INTO eb_user2usergroup(userid,groupid,createdby,createdat) SELECT in_id,groupid,in_userid,SYSTIMESTAMP FROM 
                (SELECT regexp_substr(in_group_temp,'[^,]+', 1, level) AS groupid from dual CONNECT BY regexp_substr(in_group_temp, '[^,]+', 1, level) is not null MINUS SELECT TO_CHAR(groupid) from eb_user2usergroup WHERE userid = in_id AND eb_del = 'F');
        END IF;        
    ELSE        
        ...
        IF in_roles_temp IS NOT NULL THEN
            INSERT INTO eb_role2user (role_id,user_id,createdby,createdat) SELECT roleid, out_usid,in_userid,SYSTIMESTAMP 
                 FROM (SELECT regexp_substr(in_roles_temp,'[^,]+', 1, level) AS roleid from dual CONNECT BY regexp_substr(in_roles_temp, '[^,]+', 1, level) is not null);
        END IF;
        IF in_group_temp IS NOT NULL THEN
            INSERT INTO eb_user2usergroup(userid,groupid,createdby,createdat) SELECT out_usid, groupid,in_userid,SYSTIMESTAMP 
                 FROM (SELECT regexp_substr(in_group_temp,'[^,]+', 1, level) AS groupid from dual CONNECT BY regexp_substr(in_group_temp, '[^,]+', 1, level) is not null);
        END IF;
    END IF;
    IF in_id > 0 THEN
        UPDATE eb_usersanonymous SET modifiedby = in_userid, modifiedat = SYSTIMESTAMP, ebuserid = out_usid WHERE id = in_anonymoususerid;
    END IF;
    COMMIT;
    RETURN out_usid;
END;