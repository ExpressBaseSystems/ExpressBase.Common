...
BEGIN
	...
	IF _id > 0 THEN
		...
		INSERT INTO eb_user2usergroup
			...
			SELECT userid,_id,_userid,NOW() FROM 
		...
		WHERE 
			groupid = _id AND eb_del = 'F' AND userid IN(
						SELECT UNNEST(array(
								SELECT userid from eb_user2usergroup WHERE groupid = _id AND eb_del = 'F')) except 
											SELECT UNNEST(users));	
	ELSE
		...
		INSERT INTO eb_user2usergroup(userid,groupid,createdby,createdat) 
			SELECT 
					userid, gid,_userid,NOW() 
    			FROM 
					UNNEST(users) AS userid;
	END IF;
