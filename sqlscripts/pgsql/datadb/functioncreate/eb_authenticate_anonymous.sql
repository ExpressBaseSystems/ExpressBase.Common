...
CREATE OR REPLACE FUNCTION public.eb_authenticate_anonymous(
    ...
    RETURNS TABLE(out_userid integer, out_status_id integer, out_email text, out_fullname text, out_roles_a text, out_rolename_a text, out_permissions text, out_preferencesjson text, out_constraints_a text, out_signin_id integer, out_usergroup_a text, out_public_ids text, out_user_type integer) 
    LANGUAGE 'plpgsql'
...
IF in_socialid IS NOT NULL THEN
    ...
    SELECT 
			_userid, _status_id, _email, _fullname, _roles_a, _rolename_a, _permissions, _preferencesjson, _constraints_a, _signin_id, _usergroup_a, _public_ids, _user_type
		FROM 
			eb_authenticate_unified(social => in_socialid, wc => in_wc, ipaddress => in_user_ip) 
		INTO out_userid, out_status_id, out_email, out_fullname, out_roles_a, out_rolename_a, out_permissions, out_preferencesjson, out_constraints_a, out_signin_id, out_usergroup_a, out_public_ids, out_user_type;
...
IF is_anon_auth_req THEN
	SELECT _userid, _email, _status_id, _fullname, _roles_a, _rolename_a, _permissions, _preferencesjson, _constraints_a, _signin_id, _usergroup_a, _public_ids, _user_type
    ...
    INTO out_userid, out_email, out_status_id, out_fullname, out_roles_a, out_rolename_a, out_permissions, out_preferencesjson, out_constraints_a, out_signin_id, out_usergroup_a, out_public_ids, out_user_type;
END IF;

RETURN QUERY
    SELECT out_userid, out_status_id, out_email, out_fullname, out_roles_a, out_rolename_a, out_permissions, out_preferencesjson, out_constraints_a, out_signin_id, out_usergroup_a, out_public_ids, out_user_type;
