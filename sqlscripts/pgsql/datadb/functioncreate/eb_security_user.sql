...
BEGIN
	...
	IF _id > 1 THEN
		...
		UPDATE eb_users 
		SET 
			fullname= _fullname, nickname=_nickname, email=_email, dob=_dob, sex=_sex, alternateemail=_alternateemail, phnoprimary=_phnoprimary, phnosecondary=_phnosecondary, landline=_landline, phextension=_phextension, fbid=_fbid, fbname=_fbname, statusid=_statusid, hide=_hide, preferencesjson=_preferences 
		WHERE 
			id = _id;
			
		INSERT INTO eb_role2user(role_id,user_id,createdby,createdat) 
			SELECT 
					roleid,_id,_userid,NOW() 
				FROM 
					UNNEST(array(SELECT unnest(_roles) 
					except 
		...
		WHERE 
			user_id = _id AND eb_del = 'F' AND role_id IN(
				SELECT UNNEST(array(SELECT role_id from eb_role2user WHERE user_id = _id AND eb_del = 'F')) 
				except 
					SELECT UNNEST(_roles));

		INSERT INTO eb_user2usergroup(userid,groupid,createdby,createdat) 
		SELECT 
				_id,groupid,_userid,NOW() 
			FROM 
				UNNEST(array(SELECT unnest(_group) 
				except 
		...
		SET 
			eb_del = 'T',revokedby = _userid,revokedat =NOW() 
		WHERE 
			userid = _id AND eb_del = 'F' AND groupid IN(
	...
	ELSE
		...
		INSERT INTO eb_users 
			(fullname, nickname, email, pwd, dob, sex, alternateemail, phnoprimary, phnosecondary, landline, phextension, fbid, fbname, createdby, createdat, statusid, hide, preferencesjson) 
		VALUES 
			(_fullname, _nickname, _email, _pwd, _dob, _sex, _alternateemail, _phnoprimary, _phnosecondary, _landline, _phextension, _fbid, _fbname, _userid, NOW(), _statusid, _hide, _preferences) RETURNING id INTO uid;
      
		INSERT INTO eb_role2user 
			(role_id,user_id,createdby,createdat) 
		SELECT 
				roleid, uid,_userid,NOW() 
			FROM 
				UNNEST(_roles) AS roleid;
    
		INSERT INTO eb_user2usergroup
				(userid,groupid,createdby,createdat) 
			SELECT uid, groupid,_userid,NOW() 
				FROM UNNEST(_group) AS groupid;
	
		IF _id > 0 THEN
			UPDATE eb_usersanonymous 
			SET 
				modifiedby = _userid, modifiedat = NOW(), ebuserid = uid
			WHERE 
				id = _anonymoususerid;
		END IF;
