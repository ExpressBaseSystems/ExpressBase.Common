...
CREATE OR REPLACE FUNCTION public.eb_authenticate_unified(
    ...
    RETURNS TABLE(_userid integer, _status_id integer, _email text, _fullname text, _roles_a text, _rolename_a text, _permissions text, _preferencesjson text, _constraints_a text, _signin_id integer, _usergroup_a text, _public_ids text, _user_type integer) 
    LANGUAGE 'plpgsql'
...
BEGIN
	...
	IF uname IS NOT NULL AND password IS NOT NULL AND social IS NULL THEN
        SELECT id, email, fullname, preferencesjson, eb_user_types_id, statusid FROM eb_users 
		WHERE email = uname AND pwd = password AND (statusid = 0 OR statusid = 4) INTO _userid, _email, _fullname, _preferencesjson, _user_type, _status_id;
    ...
    IF uname IS NOT NULL AND password IS NULL AND social IS NULL THEN
        SELECT id, email, fullname, preferencesjson, eb_user_types_id, statusid FROM eb_users 
		WHERE email = uname AND (statusid = 0 OR statusid = 4) INTO _userid, _email, _fullname, _preferencesjson, _user_type, _status_id;
		IF _userid IS NULL THEN	
    ...
    IF uname IS NULL AND password IS NULL AND social IS NOT NULL THEN
        SELECT id, email, fullname, preferencesjson, eb_user_types_id, statusid FROM eb_users 
		WHERE fbid = social AND (statusid = 0 OR statusid = 4) INTO _userid, _email, _fullname, _preferencesjson, _user_type, _status_id;
		IF _userid IS NULL THEN	
	...
	IF _status_id IS NULL THEN
		_status_id := 0;
	END IF;
    RETURN QUERY SELECT _userid, _status_id, _email, _fullname, _roles_a, _rolename_a, _permissions, _preferencesjson, _constraints_a, _signin_id, _usergroup_a, _public_ids, _user_type;
END;
