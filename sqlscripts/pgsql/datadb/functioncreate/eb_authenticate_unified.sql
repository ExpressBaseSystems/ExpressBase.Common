...
BEGIN
    ...
    IF uname IS NOT NULL AND password IS NULL AND social IS NULL THEN
        SELECT id, email, fullname, preferencesjson FROM eb_users 
		WHERE email = uname AND statusid = 0 INTO _userid, _email, _fullname, _preferencesjson;
		IF _userid IS NULL THEN	
    ...
    IF uname IS NULL AND password IS NULL AND social IS NOT NULL THEN
        SELECT id, email, fullname, preferencesjson FROM eb_users 
		WHERE fbid = social AND statusid = 0 INTO _userid, _email, _fullname, _preferencesjson;
		IF _userid IS NULL THEN	
