...
create or replace FUNCTION eb_object_create_minor_version(
    ...
    refidunique VARCHAR(500); inserted_objid CLOB; inserted_obj_ver_id NUMBER; objid NUMBER; committed_refidunique CLOB; 
	minor NUMBER;major NUMBER; version_number CLOB; objjson CLOB;

BEGIN

	SELECT eb_objects_id, major_ver_num,obj_json into objid, major,objjson FROM eb_objects_ver WHERE refid=idv;

	SELECT MAX(minor_ver_num) into minor FROM eb_objects_ver WHERE eb_objects_id = objid AND major_ver_num = major;

    version_number := CONCAT(CONCAT(CONCAT(CONCAT(CONCAT(CONCAT(major,'.'),minor+1),'.'),0),'.'),'w');
