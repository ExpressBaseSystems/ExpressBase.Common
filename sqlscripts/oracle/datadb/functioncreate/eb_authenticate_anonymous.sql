...
BEGIN
    ...
    IF in_socialid IS NOT NULL THEN  
        ...
        SELECT userid, email, fullname, roles_a, rolename_a, permissions, preferencesjson 
            INTO out_userid, out_email, out_fullname, out_roles_a, out_rolename_a, out_permissions,out_preference 
            FROM table(eb_authenticate_unified(social => in_socialid, wc => in_wc));         
        ...
        IF out_userid IS NULL THEN
            ...
            SELECT A.id, A.email, A.fullname INTO out_userid, out_email, out_fullname FROM eb_usersanonymous A 
                WHERE A.socialid = in_socialid AND appid = in_appid;
                EXCEPTION WHEN  NO_DATA_FOUND THEN 
                    out_userid := NULL;
                    out_email := NULL;
                    out_fullname := NULL;
            ...
            ELSE
                ...
                UPDATE eb_usersanonymous SET lastvisit = SYSTIMESTAMP, totalvisits = totalvisits + 1, ipaddress = in_user_ip, browser = in_user_browser, city = in_city, region = in_region, country = in_country, latitude = in_latitude, longitude = in_longitude, timezone = in_timezone, iplocationjson = in_iplocationjson 
                    WHERE id = out_userid;        
            END IF;
            is_anon_auth_req := 'T';            
        END IF; 
        ...
        IF in_emailid IS NOT NULL OR in_phone IS NOT NULL THEN
            ...
            ELSE
                IF out_email IS NULL THEN
                    UPDATE eb_usersanonymous SET email = in_emailid, lastvisit = SYSTIMESTAMP, totalvisits = totalvisits + 1, ipaddress = in_user_ip, browser = in_user_browser, city = in_city, region = in_region, country = in_country, latitude = in_latitude, longitude = in_longitude, timezone = in_timezone, iplocationjson = in_iplocationjson 
                        WHERE phoneno = in_phone;
                    --UPDATE eb_usersanonymous SET email = in_emailid, lastvisit = SYSTIMESTAMP, totalvisits = totalvisits + 1 WHERE phoneno = in_phone;
                ELSE
                    UPDATE eb_usersanonymous SET phoneno = in_phone, lastvisit = SYSTIMESTAMP, totalvisits = totalvisits + 1, ipaddress = in_user_ip, browser = in_user_browser, city = in_city, region = in_region, country = in_country, latitude = in_latitude, longitude = in_longitude, timezone = in_timezone, iplocationjson = in_iplocationjson 
                        WHERE email = in_emailid;  
    ...
    IF is_anon_auth_req = 'T' THEN
        SELECT email, fullname, roles_a, rolename_a, permissions, preferencesjson INTO out_email, out_fullname, out_roles_a, out_rolename_a, out_permissions, out_preference
