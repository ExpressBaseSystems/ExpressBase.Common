	...
	IF uname IS NOT NULL AND pwd IS NOT NULL AND social IS NULL THEN
        SELECT 
			...
			FROM 
				eb_users EU
			WHERE 
				EU.email = uname AND EU.pwd = pwd AND EU.statusid = 0 
			INTO userid, email, fullname, preferencesjson;
            ...
            IF userid IS NULL THEN
				SELECT 
					...
					FROM 
						eb_users EUS
					WHERE 
						EUS.email = uname AND EUS.statusid = 0 
					INTO userid;
                IF userid > 0 THEN
					INSERT INTO 
	...
	IF uname IS NOT NULL AND pwd IS NULL AND social IS NULL THEN
        SELECT 
			...
			FROM 
				eb_users EU
			WHERE 
				EU.email = uname AND EU.statusid = 0 
			INTO userid, email, fullname, preferencesjson;
        IF userid IS NULL THEN	
    ...
    IF uname IS NULL AND pwd IS NULL AND social IS NOT NULL THEN
        SELECT 
			...
			FROM 
				eb_users EU 
			WHERE 
				EU.fbid = social AND EU.statusid = 0 
			INTO userid, email, fullname, preferencesjson;
        IF userid IS NULL THEN	
