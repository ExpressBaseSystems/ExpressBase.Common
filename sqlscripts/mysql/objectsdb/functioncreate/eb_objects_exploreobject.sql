-- FUNCTION AS PROCEDURE: eb_objects_exploreobject(integer)

-- DROP PROCEDURE IF EXISTS eb_objects_exploreobject;

DELIMITER $$       
CREATE PROCEDURE eb_objects_exploreobject(_id integer)
BEGIN
	...
	SELECT idv, namev, typev, status, description, changelog, commitat, commitby, refidv, ver_num, COALESCE(work_mode,'F'), workingcopies,
		...
		FROM 
			(SELECT eb_objects_getversiontoopen(_id)) AS A
		INTO
